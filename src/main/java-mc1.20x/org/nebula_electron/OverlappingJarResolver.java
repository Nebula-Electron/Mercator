...
/**
 * Fixes duplicate mod-ID crashes for MC 1.20.x (securejarhandler 3.x).
 *
 * <p>Some mods ship an outer jar that embeds a JarInJar inner jar, both with the same
 * {@code modId}. When both get loaded a duplicate-mod error kills startup. This class
 * detects that case and returns the inner jar directly instead. Unlike the 1.21.x version,
 * {@code securejarhandler 3.x} has no {@code ofFilteredPaths}, so a filtered overlay isn't
 * an option here.
 *
 * <p>Jars with no JarJar metadata, or whose inner jar has a different mod ID, come back
 * unchanged via {@link JarContents#of}.
 */
